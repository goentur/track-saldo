...
function Index({ pemiliks }){
    const route = useRoute();
    const { delete:destroy } = useForm()
    const [isLoading, setIsLoading] = useState(false);
    const perPage = useRef(pemiliks.per_page);
    ...
    const getData = () => {
        setIsLoading(true);
        router.get(route('master.pemilik.index',pickBy({
    ...
    const hapus = (id) => {
        withReactContent(Swal).fire({
            ...
            if (result.isConfirmed) {
                destroy(route('master.pemilik.destroy',id))
            }
        })
    }
    return (
        <Layout>
            ...
            <Card>
                <CardHeader className="d-flex justify-content-between align-items-center">
                    <CardTitle><FontAwesomeIcon icon={faUsers}/> PEMILIK</CardTitle>
                    <Link href={route('master.pemilik.create')} className="btn btn-primary"><FontAwesomeIcon icon={faPlus}/> TAMBAH DATA</Link>
                </CardHeader>
                <CardBody>
                    ...
                    <Table bordered hover size="sm">
                        ...
                        <tbody>
                            ...
                            <tr key={pemilik.id}>
                                ...
                                <td>{ pemilik.name }</td>
                                <td>
                                    { pemilik.toko.map(toko => (
                                            <span className="badge me-1 bg-primary" key={pemilik.id+'-'+toko.id}>{toko.nama}</span>
                                        )) 
                                    }
                                </td>
                                <td>
                                    <ButtonGroup aria-label="button action">
                                        <Link href={route('master.pemilik.edit',pemilik)} className="btn btn-sm btn-success"><FontAwesomeIcon icon={faEdit}/></Link>
                                        <Button size="sm" variant="danger" onClick={() => hapus(pemilik.id)}><FontAwesomeIcon icon={faTrash}/></Button>
                                    </ButtonGroup>
                                </td>
                            </tr>
                            ))}
                    ...
                    <div className="row">
                        <div className="col-lg-4">Menampilkan {pemiliks.from} sampai {pemiliks.to} dari {pemiliks.total} data</div>
                        <div className="col-lg-8"><Pagination datas={pemiliks.links}/></div>
                    </div>
                </CardBody>
            </Card>
        </Layout>
