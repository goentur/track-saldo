...
function Konsumtif({ toko, tanggalTransaksi, onProcessingDone }) {
    const route = useRoute();
    const [loading, setLoading] = useState(false);
    const [tabungans, setTabungan] = useState([]);
    const [dataKonsumtif, setDataKonsumtif] = useState(null);
    ...
    const getDataKonsumtif = async () => {
        setLoadingCari(true);
        try {
            const response = await axios.post(route('transaksi.pengeluaran.konsumtif.data'), { 
                toko : toko,
                tanggal : dateRange[0]?format(dateRange[0],'dd-MM-yyyy') + ' - ' + format(dateRange[1],'dd-MM-yyyy'):tanggalTransaksi,
            });
            setDataKonsumtif(response.data);
        } catch (error) {
    ...
    const formik = useFormik({
        ...
        onSubmit: async (values) => {
            setLoading(true)
            try {
                const response = await axios.post(route('transaksi.pengeluaran.konsumtif.simpan'), values);
    ...
    return (<>
        ...
        <div className="text-center">
            <span className="fs-4"><FontAwesomeIcon icon={faHistory}/> RIWAYAT PENGELUARAN KONSUMTIF</span>
            <br />
            <span className="fs-6">{loadingCari?'..':dataKonsumtif?.tanggalAwal ?? '-'} S.D {loadingCari?'..':dataKonsumtif?.tanggalAkhir ?? '-'}</span>
        </div>
        <div className="row">
            <div className="col-lg-6">
                <Form onSubmit={(e) => {e.preventDefault(); getDataKonsumtif()}} className="row mb-2">
            ...
            <div className="col-lg-6 text-end fs-4"><b>TOTAL : </b>Rp { dataKonsumtif?.total ?? 0}</div>
        </div>
        <Table bordered hover size="sm" className="f-14">
            <thead>
                ...
                <tbody>
                    ...
                    {dataKonsumtif !== null ? dataKonsumtif.data.map((value,index) => (
                    <tr key={index}>
